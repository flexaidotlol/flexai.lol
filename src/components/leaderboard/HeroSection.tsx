...
export const HeroSection: React.FC<HeroSectionProps> = ({ categories, topProduct, allProducts = [] }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. If existing product is detected (Outbid flow)
      if (detectedProduct?.found && detectedProduct.product) {
        const res = await fetch('/api/bids/create-checkout', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            product_id: detectedProduct.product.id,
            amount_cents: Math.round(userBidDollars * 100),
          }),
        });

        const data = await res.json();
        if (!res.ok || data.error) {
          throw new Error(data.error || 'Failed to create outbid checkout');
        }
      ...
      // 2. New product submission flow - directly create Stripe checkout
      let targetCategoryId = categories[0]?.id || 'all';
      if (selectedCategory && selectedCategory !== 'all' && !isCustomCategory) {
      ...
      const payload = {
        name: cleanName,
        tagline: 'Verified AI Product & Software',
        website_url: inputValue.trim().startsWith('http') ? inputValue.trim() : `https://${inputValue.trim()}`,
      ...
      <div className="relative text-center max-w-3xl mx-auto space-y-3">
        ...
        >
          ...
          <button
            ...
            className={`w-full sm:w-auto px-5 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all whitespace-nowrap flex items-center justify-center gap-2 ${
              isFormReady && !isSubmitting
                ? 'text-white bg-gradient-to-r from-purple-600 via-indigo-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 shadow-glow-purple border border-purple-400/30 cursor-pointer hover:scale-[1.01] active:scale-[0.99]'
        ...
        {/* Dynamic Error Message if Checkout API returns issue */}
        {errorMessage && (
          <div className="p-3 max-w-xl mx-auto rounded-xl bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 text-xs flex items-center justify-center gap-2 animate-fadeIn">
